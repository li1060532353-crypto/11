...
function mockDb(rows: Row[] = [], fail = false) {
  ...
  const prepare = (sql: string) => ({ bind: (...values: unknown[]) => ({
    ...
    async run() { if (fail) throw new Error('db'); return { success: true }; },
    sql, values,
  }) });
  return { db: { prepare, async batch(statements: Array<{ sql: string; values: unknown[] }>) { if (fail) throw new Error('db'); batches.push(statements); return []; } } as unknown as D1Database, batches };
}
...
describe('notes Pages route', () => {
  ...
  it('creates exactly a note/version batch and rejects malformed bodies', async () => {
    const mock = mockDb();
    const response = await notesRoute(context(new Request('http://x/api/notes', { method: 'POST', body: JSON.stringify(body) }), mock.db));
    expect(response.status).toBe(201); expect(mock.batches).toHaveLength(1); expect(mock.batches[0]).toHaveLength(2); expect(mock.batches[0].map((statement) => statement.sql)).toEqual(expect.arrayContaining([expect.stringContaining('INSERT INTO notes'), expect.stringContaining('INSERT INTO note_versions')]));
  ...
  it('patches partially, rejects empty bodies, handles missing, and archives without delete', async () => {
    ...
    expect(mock.batches[0]).toHaveLength(2);
