...
function isAsset(value: unknown): value is AssetRecord { const item = record(value); const size = item?.sizeBytes; return !!item && assetId(item.id) && (item.noteId === null || assetId(item.noteId)) && safeAssetName(item.originalName) && string(item.mimeType) && assetTypes.has(item.mimeType) && typeof size === 'number' && Number.isSafeInteger(size) && size >= 0 && size <= 15 * 1024 * 1024 && date(item.createdAt); }
